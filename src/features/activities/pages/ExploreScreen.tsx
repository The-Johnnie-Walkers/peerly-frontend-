...
type SortKey = 'date' | 'places';
const SORT_OPTIONS: { key: SortKey; label: string }[] = [
  { key: 'date', label: 'Más próximas' },
  { key: 'places', label: 'Más cupos' },
];
...
const ExploreScreen = () => {
  ...
  const filteredActivities = useMemo(() => {
    ...
    if (sort === 'places') {
      return [...filtered].sort((a, b) => (b.availablePlaces ?? 0) - (a.availablePlaces ?? 0));
    }
    return filtered;
  }, [activities, userActivityIds, deferredSearch, sort]);
  ...
  return (
    <>
      <div className="min-h-svh bg-background px-4 py-6 sm:px-5 sm:py-5 lg:px-6 lg:py-2">
        <div className="min-h-full rounded-[28px] bg-background md:rounded-[32px] md:shadow-elevated">
          <div className="mx-auto flex w-full max-w-[1440px] flex-col gap-8 px-6 py-8 pb-24 sm:px-8 sm:py-9 lg:gap-10 lg:px-10 lg:py-10 xl:px-12 2xl:px-14">
            <header className="flex flex-col gap-5 rounded-[32px] border border-white/70 bg-[linear-gradient(135deg,rgba(255,255,255,0.96),rgba(246,236,227,0.88))] px-7 py-8 shadow-card sm:px-8 sm:py-9 lg:flex-row lg:items-center lg:justify-between lg:px-10">
            ...
            {/* Buscador — visible desde el inicio en mobile */}
            <div className="relative">
              ...
              <input
                ...
                placeholder="Buscar actividades disponibles por nombre, lugar o descripción"
            ...
            <section className="space-y-6">
              <div className="rounded-[30px] border border-white/70 bg-white/72 px-5 py-5 shadow-card sm:px-6 sm:py-6 ">
                  ...
                  <div className="flex items-center gap-1.5">
                    {SORT_OPTIONS.map(({ key, label }) => (
                      <button
                        ...
                        type="button"
                        onClick={() => setSort(key)}
                        className={`h-8 rounded-full px-3 text-[11px] font-mono font-bold uppercase tracking-wider transition-colors ${
                          sort === key
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.95 }}
        onClick={() => navigate('/create-activity')}
        className="fixed bottom-6 right-6 z-40 lg:hidden inline-flex h-14 w-14 items-center justify-center rounded-full bg-[hsl(var(--peerly-primary))] text-white shadow-elevated"
